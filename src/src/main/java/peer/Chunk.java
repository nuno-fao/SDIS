...
public class Chunk {
    ...
    public Chunk(int chunkNo, String fileId, int repDegree,String serverName) {
    ...
    Chunk(int chunkNo, String fileId, int repDegree, int size,String serverName) {
        this.chunkNo = chunkNo;
        this.fileId = fileId;
        this.repDegree = repDegree;
        this.peerCount = new ConcurrentHashMap<>();
    ...
    public Chunk(int chunkNo, String fileId, int repDegree, int realDegree, int size,String serverName) {
        this.chunkNo = chunkNo;
        this.fileId = fileId;
        this.repDegree = repDegree;
    ...
    synchronized void updateRdata() {
        if (repDegree != -1) {
            Path path = Paths.get(serverName + "/.rdata/" + this.fileId + "/" + this.chunkNo);
    ...
    synchronized void updateLdata(String filename) {
        if (repDegree != -1) {
            Path path = Paths.get(serverName + "/.ldata/" + this.fileId + "/" + this.chunkNo);
            AsynchronousFileChannel fileChannel = null;
            try {
                fileChannel = AsynchronousFileChannel.open(
                        path, WRITE, TRUNCATE_EXISTING, CREATE);
